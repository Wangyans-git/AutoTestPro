...
"""
/*设置待机操作标志的状态:
    operation_off:0x00(断电上电或倾倒保护或OTA重启),0x01(用户全状态上报的关机，包括24H关机，按键延时关机),
    0x02(用户APP操作关机),0x03(模块延时关机),0x04(模块定时关机),0x05(恒温待机),0x06(用户开机),0x07(恒温开机)
    */
"""
...
class Mqtt_Utils:
    ...
    def __init__(self, log_file_in_name, out_file_name):
        # LOG_FILE_NAME = str(Path(ROOT) / "tool/in_log.txt")
        LOG_FILE_NAME = log_file_in_name
        DECODE_FILE_NAME = out_file_name
        try:
...
class Special_BLE:
    ...
    def Special_BLE_Thermostat_prase(self, cmd, data):
        ...
        if ((data[6] & 0x80) >> 7):
            ...
            info += "温度计"
        temp = ((data[7] & 0x7F) << 16) + (data[8] << 8) + data[9]
        humi = temp % 1000
        temper = (temp - humi) / 1000
        if (data[7] >> 7):
            temper = -temper
    ...
    def Special_BLE_WorkMode_prase(self, cmd, data):
        info = ""
        if (data[2] == 0x01):
            info += "立式 "
        elif (data[2] == 0x02):
            ...
            for i in info:
                ...
                if (cmd in BLE_Special_Protocol):
                    if (cmd == "0x05"):
                        ...
                        rst = self.ble_specail.Special_BLE_WorkMode_prase(cmd, base64_data)
                    self.utils.output_to_file(rst)
...
def Log_Prase_Handle():
    ...
    for file_name in file_list:
        ...
        out_file_name = "../解析数据/" + file_name.split('.')[0] + '_' + '解析日志.txt'
