...
class H7124_Wifi:
    def __init__(self, com, com1, dbs, dbs1, timeout):
        ...
        # self.device = u2.connect_usb('R5CR20H14AV')
        ...
        path = str(Path(ROOT) / "H7124/logs/H7124wifi配网压测.log")
        print(path)
    ...
    def watch(self):
        while True:
            ...
            if self.device(text='知道了').exists():
                self.device(text='知道了').click_exists(timeout=5)
            else:
                pass
    ...
    def add_devise_devices(self):
        ...
        while True:
            try:
                """添加设备"""
                # 添加”+“
                if self.device(resourceId="com.govee.home:id/ivDevAdd").exists(timeout=30):
                    ...
                    while True:
                        ...
                        if self.device(text='H7124_91EA').exists(timeout=30):
                            break
                        else:
                            self.device(text='重新扫描').click_exists(timeout=10)
                    # 选择设备  H5086_681B   H5086_67c9
                    while True:
                        self.device(text='H7124_91EA').click_exists(timeout=60)
                        print("sku点不到了")
                        time.sleep(1)
                ...
                # wifi配置
                # if self.device(text='Govee-2.4g').exists(timeout=30):
                ...
                # 跳过配网
                if self.device(resourceId='com.govee.home:id/skip').exists(timeout=30):
                    while True:
                        print("跳过")
                        self.device(resourceId="com.govee.home:id/skip").click_exists(timeout=5)
                        self.device(resourceId="com.govee.home:id/btn_done").click_exists(timeout=5)
                        if self.device(resourceId="com.govee.home:id/iv_switch").exists(timeout=60):
                            break

                # if self.device(text='ASUS_F0_2G').exists(timeout=30):
                #     self.device(resourceId="com.govee.home:id/et_pwd").clear_text()
                ...
                # 设置
                if self.device(resourceId="com.govee.home:id/ivRightMost").exists():
                    print("删除设备")
                    self.device(resourceId="com.govee.home:id/ivRightMost").click_exists(timeout=30)
                    """ 删除设备 """
                    time.sleep(2)
                    self.down()
                    time.sleep(2)
