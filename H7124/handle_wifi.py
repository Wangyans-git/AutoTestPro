...
class H7124_Wifi:
    def __init__(self, com, com1, dbs, dbs1, timeout):
        ...
        path = str(Path(ROOT) / "logs/H7124wifi配网压测.log")
            ...
            try:
                ...
                if self.device(resourceId="com.govee.home:id/ivDevAdd").exists(timeout=60):
                    ...
                    while True:
                        ...
                        time.sleep(2)
                        if self.device(text='H7124_905B').exists(timeout=30):
                            break
                    # 选择设备  H5086_681B   H5086_67c9
                    while True:
                        self.device(text='H7124_905B').click_exists(timeout=60)
                        print("sku点不到了")
                        time.sleep(1)
