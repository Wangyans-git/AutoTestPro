...
class SerialAuto(object):
    ...
    def date_result(self, n):
        ...
        if len(date) - 2 == len(n):  # 因为split分隔后会多一段数据，所以-1
            ...
            err_ = str(datetime.now().strftime("%Y-%m-%d %H:%M:%S前的一次执行中有错误！！！"))
            self.txt_date += err_
            self.write_txt(self.txt_date)
            print(err_)
    ...
    def run_test(self, test_list):
        ...
        while True:
            ...
            for i in range(len(test_list)):
                program.write_date(test_list[i]+"\r")  # 写入数据
                self.get_log.info("输入{0}".format(test_list[i]))
                print("输入{0}".format(test_list[i]))
                time.sleep(1)
            time.sleep(1)
            print("========第{}次测试完成========".format(m + 1))
            self.get_log.info("========第{}次测试完成========".format(m + 1))
            # program.date_result(test_list)
            m += 1


if __name__ == '__main__':
    program = SerialAuto('com3', 115200, 3)
    ...
    if program.err == 0:
        ...
        send_list = [
            'iot_test 01 01',  # 开机
            'iot_test 02 01 01', 'iot_test 02 01 02', 'iot_test 02 01 03',  # 风扇档位
            'iot_test 02 02 00', 'iot_test 02 02 02', 'iot_test 02 02 03',
            'iot_test 02 02 04', 'iot_test 02 02 05',  # 自动
            'iot_test 02 03 00', 'iot_test 02 04 00',  # 睡眠/暴风
            # 'iot_test 07 01\r', 'iot_test 07 00\r',  # 指示灯
            'iot_test 12 00', 'iot_test 12 01',  # 夜灯
            'iot_test 04 00 60', 'iot_test 04 00 00',  # 延时关闭
            'iot_test 01 00'   # 关机
        ]
        program.run_test(send_list)
