...
class H7135_OTA(object):
    ...
    def __init__(self, com, dbs, timeout, update_count):
        self.com = com
        self.dbs = dbs
    ...
    def read_date(self):
        ...
        with open(f'logs\\串口日志记录{time_log}', 'w') as file_handle:
            while row_num <= self.update_count:
                try:
                    date_line = self.ser.readline().decode().replace("\r","")
                    file_handle.write(date_line)
                    if "Number of blocks remaining: 870" in date_line:
                        row_num += 1
                        print("开始")
                    ...
                    elif "[28f2f9e94fdc9d40b19cdc44469a341c]" in date_line:
                        print("结束")
                        ...
                        self.sheet.cell(row_num + 1, 6).value = f"{self.success_count / self.start_count * 100:.2f}%"
                        self.filename.save(self.result_file)
                    if "Number of blocks remaining:" in date_line:
                        print(date_line)
                    match = re.search(r'\[(\w{32})]',date_line)
                    print(f"Ota_DoCalculate_MD5:{match} ")
                    if match:
                        self.sheet.cell(row_num + 1, 7).value = match
                        self.filename.save(self.result_file)
                except Exception as e:
                    # print(e)
                    pass
...
if __name__ == '__main__':
    ...
    program = H7135_OTA('com25', 115200, 3, start_ota_count)
    program.read_date()
    m = 0  # 统计测试次数
