...
class H7142Test:
    def __init__(self):
        ...
        # 脚本日志
        self.get_log = GetLog("C:\wys\AutoTestProjects\H7142_Auto\handle_one\log.log")
        # 获取手机分辨率
        self.width, self.height = self.device.window_size()

    def start_test(self):
        # 判断当前是否需要进入详情页
        if self.device(text='Smart Humidifier').wait(timeout=5.0):
            self.device(text='Smart Humidifier').click_exists(timeout=5.0)
            time.sleep(5)
