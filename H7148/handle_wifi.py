...
class H7148_Wifi:
    def __init__(self, com, com1, dbs, dbs1, timeout):
        ...
        path = str(Path(ROOT) / "H7148/logs/H7148wifi配网压测.log")
        print(path)
            ...
            try:
                ...
                if self.device(resourceId="com.govee.home:id/ivDevAdd").exists(timeout=10):
                    ...
                    while True:
                        ...
                        time.sleep(2)
                        if self.device(text='H7148_DB7A').exists(timeout=10):
                            break
                        else:
                            self.device(text='重新扫描').click_exists(timeout=10)
                    # 选择设备  H5086_681B   H5086_67c9
                    while True:
                        self.device(text='H7148_DB7A').click_exists(timeout=10)
                        print("sku点不到了")
                        time.sleep(1)
                ...
                #         if self.device(resourceId="com.govee.home:id/iv_switch").exists(timeout=10):
                #             break
    ...
    def error_handle(self):
        if self.device(resourceId="com.govee.home:id/iv_switch").wait(timeout=10.0):
            ...
            # 退出详情页
            try:
                self.device(resourceId="com.govee.home:id/btn_back").click_exists(timeout=10.0)
                print("退出详情页")
    ...
    def del_device(self):
        # 设置
        if self.device(resourceId="com.govee.home:id/btn_setting").exists(timeout=10):
            print("删除设备")
            ...
            while True:
                self.down()
                self.device(resourceId="com.govee.home:id/btn_delete").click_exists(timeout=10)
                time.sleep(2)
